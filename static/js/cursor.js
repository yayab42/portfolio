...
function move(e){
  for (var ii = 1; ii < numP; ii++) {
    TweenMax.to('#p'+ii, .5,{delay:.005*ii, bezier:{type:'soft', values:[{x:Math.max(Math.min(e.clientX+25-50*Math.random(), window.innerWidth-21), 21),
                                                                          y:Math.max(Math.min(e.clientY+25-50*Math.random(), window.innerHeight-21), 21)}, 
                                                                         {x:Math.max(Math.min(e.clientX, window.innerWidth-21), 21), 
                                                                          y:Math.max(Math.min(e.clientY,window.innerHeight-21),21)}]}, 
